// LeetCode Question URL: https://leetcode.com/problems/minimum-absolute-difference-in-bst/
...
class Solution2 {
    public int getMinimumDifference(TreeNode root) {
        ...
        getMinimumDifferenceHelper(root, pre, minDiff);

        return minDiff[0];
    }

    private void getMinimumDifferenceHelper(TreeNode node, TreeNode[] pre, int[] minDiff) {
        if (node == null) {
            return;
        }

        getMinimumDifferenceHelper(node.left, pre, minDiff);
        ...
        getMinimumDifferenceHelper(node.right, pre, minDiff);
    }
}
